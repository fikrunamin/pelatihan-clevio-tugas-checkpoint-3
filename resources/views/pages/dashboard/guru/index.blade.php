            ...
            <div class="bg-white p-10">
                <div class="mb-10">
                    <h3 class="mb-2"><a href="{{route('dashboard.index')}}"
                                        class="border-b-2 border-color-1">Dashboard</a> / Guru</h3>
                    <h1 class="text-2xl font-semibold">Daftar Guru</h1>
                </div>
                <div class="grid grid-cols-1">
                    <div class="flex justify-end gap-x-5 mb-5">
                        <a href="{{route('guru.create')}}" class="flex items-center py-1 px-3 bg-color-1 text-color-4"><i
                                class="material-icons">add</i> Tambah Guru</a>
                    </div>
                    <div class="grid grid-cols-1">
                        <table id="table" class="text-center">
                            ...
                            @foreach($list_guru as $index => $guru)
                                <tr>
                                    <td>{{$index + 1}}</td>
                                    <td><img src="{{asset('images/' . $guru->gambar_profil)}}"
                                             class="object-cover h-16 w-16"></td>
                                    <td>{{$murid->nis ?? 'Belum ada NIM'}}</td>
                                    <td>{{$guru->nama}}</td>
                                    <td>{{$guru->mata_pelajaran->nama}} kelas {{$guru->mata_pelajaran->kelas}}</td>
                                    <td>{{$guru->updated_at->diffForHumans()}}</td>
                                    <td>
                                        <div class="flex gap-x-5">
                                            <a href="{{route('guru.show', ['guru' => $guru->id])}}"><i
                                                    class="material-icons">visibility</i></a>
                                            <a href="{{route('guru.edit', ['guru' => $guru->id])}}"><i
