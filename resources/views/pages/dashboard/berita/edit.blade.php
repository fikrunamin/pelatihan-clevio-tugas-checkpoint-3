        ...
        <div class="col-span-5 pt-10 bg-color-1 p-10">
            <div class="bg-white p-10">
                ...
                <div class="grid grid-cols-1">
                    ...
                    <div class="grid grid-cols-1">
                        <form action="{{route('berita.update', ['beritum' => $berita->id])}}" method="post" class="" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="w-full mb-3">
                                <label class="">Sampul</label><br>
                                <input type="file" class="px-3 py-2 bg-color-1 text-color-4 w-full mt-3" name="sampul" value="{{$berita->sampul}}">
                            </div>
                            <div class="w-full mb-3">
                                <label class="">Judul</label><br>
                                <input type="text" class="px-3 py-2 bg-color-1 text-color-4 w-full mt-3" name="judul" value="{{$berita->judul}}">
                            </div>
                            <div class="w-full mb-3">
                                <label class="">Penulis</label><br>
                                <input type="text" class="px-3 py-2 bg-color-1 text-color-4 w-full mt-3" name="penulis" value="{{$berita->penulis}}">
                            </div>
                            <div class="w-full mb-3">
                                <label class="">Isi</label><br>
                                <textarea name="isi" id="deskripsi">{{$berita->isi}}</textarea>
